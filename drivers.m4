...
AC_CHECK_LIB(freetype, FT_Init_FreeType, has_freetypelib="true", has_freetypelib="false")
AC_CHECK_HEADER(ft2build.h, has_freetypehdr="true", has_freetypehdr="false", [#include <ft2build.h>])
if test "$has_freetypehdr" = "true"; then
...
if test "$VNC" = "yes"; then
   if test "$has_vncserverlib" = "true"; then
      ...
      if test "$has_freetypelib" = "true"; then
         DRVLIBS="$DRVLIBS -lfreetype"
      fi
